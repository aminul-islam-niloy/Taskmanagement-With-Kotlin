...
//    }
    ...
    override fun onTaskCompletionStatusChanged(taskManagementData: TaskManagementData) {
        ...
        val updatedIsCompleted = !taskManagementData.isCompleted

        // Determine the new taskStatus based on the conditions
        val newTaskStatus = when {
            updatedIsCompleted -> "Completed"
            taskEndDateTime < currentDateTime -> "Missed"
            else -> "Pending"
        }

        // Prepare the updated values for Firebase
        ...
        databaseRef.child(taskManagementData.taskId).updateChildren(updatedValues)
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    // Update the local object and notify the adapter
                    taskManagementData.isCompleted = updatedIsCompleted
                    taskManagementData.taskStatus = newTaskStatus
                    adapter.notifyDataSetChanged()
                    Toast.makeText(context, "Task updated successfully!", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(context, "Failed to update task: ${task.exception?.message}", Toast.LENGTH_SHORT).show()
                }
            }
    }
